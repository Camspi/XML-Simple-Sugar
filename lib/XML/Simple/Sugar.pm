...
=head1 SYNOPSIS
    ...
    my $xs_2 = XML::Simple::Sugar->new({ xml => '<your_xml>here</your_xml>' });
...
=head2 MINIMUM PERL VERSION SUPPORTED
...
This is the Perl representation of the XML.  This is ugly to work with directly, hence this module.
...
This readonly attribute is only useful during instantiation (XML::Simple::Sugar->new({ xml => $xml_string })).  This is particularly useful if you are working with an XML template.
...
I wanted to write/manipulate simple XML payloads with more DWIMmery than what I found in modules available on CPAN (admittedly I didn't look very hard).  If the above syntax doesn't accomplish that for you, you probably should use a different module.

Additionally, this package depends on XML::Simple, which currently has a "do not use this module in new code" notice.  If you are cool with that, then so am I. :)
